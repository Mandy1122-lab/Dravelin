...
<body>
    ...
    <div class="container">
        ...
        if (isset($result) && $result && mysqli_num_rows($result) > 0) {
            ...
            echo '<h3 style="text-decoration: none !important;">' . $row["name"] . '</h3>';
            echo '</div></a>';
                }
        } else {
    ...
    const id_param = document.querySelector('input[name="type"]:checked').value === 'drama' ? 'd_id' : 'm_id';

    fetchSearchResults(selectedGenres, selectedType, id_param);
    });
        });
    document.addEventListener('DOMContentLoaded', function () {
...
function fetchSearchResults(selectedGenres, selectedType, id_param) {
    ...
    fetch('fetch_result.php?genre=' + encodeURIComponent(selectedGenres.join(',')) + '&type=' + encodeURIComponent(selectedType))
        ...
        .then(data => {
            if (data.length > 0) {
                let resultsHtml = '';
                data.forEach(result => {
                    let id_param = result.source == "drama" ? "d_id" : "m_id";
                    resultsHtml += `<a href="${result.source}-detail.php?${id_param}=${result.id} ">
                        <div class="card">
                            <img src="${result.img}">
                            <h3>${result.name}</h3>
                        </div>
