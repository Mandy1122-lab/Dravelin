    ...
    <section class="spad">
        <div class="container">
            <div class="row">
                ...
                <?php
                    $genre = mysqli_query($conn, "SELECT * FROM genre JOIN genrem ON genrem.g_id = genre.g_id WHERE genrem.m_id = ".$m_id."");
                    while($row = mysqli_fetch_assoc($result)){
                        $name = $row['m_name'];
                        $country = mysqli_query($conn, "SELECT c_name FROM country JOIN movie ON movie.c_id = country.c_id WHERE movie.c_id =".$row['c_id']."");
                        $year = date('Y', strtotime($row['m_date']));
                        echo"       

                            <div class='col-lg-3'>
                                <div class='anime__details__pic set-bg' data-setbg='{$row['m_pic']}'>
                                </div>
                            </div>
                            <div class='col-lg-9'>
                                                ...
                                                <ul>
                                                    ...
                                                    <li><span>類型:</span>";
                                                        while($item = mysqli_fetch_assoc($genre)){
                                                            echo"<a href='movie-genre.php?g_id={$item['g_id']}'>".$item['g_name']."</a> /";
                                                        }
                        echo"
                                                    </li>
                                                </ul>
            ...
            <div class="col-lg-12 col-md-12">
                ...
                <div class="owl-carouselSpot owl-carousel owl-theme">
                    ...
                    <?php
                        $stmt = mysqli_prepare($conn, "SELECT * FROM spot JOIN spotm ON spotm.s_id = spot.s_id WHERE spotm.m_id = ?");
                ...
                <div class="owl-carousel owl-theme">
                    <?php
                        $stmt = mysqli_prepare($conn, "SELECT * FROM actor JOIN actorm ON actor.a_id = actorm.a_id WHERE actorm.m_id = ?");
                        mysqli_stmt_bind_param($stmt, "i", $m_id);
                        mysqli_stmt_execute($stmt);
                        $result = mysqli_stmt_get_result($stmt);
                ...
                <!-- Modal -->
                <?php
                    $result2 = mysqli_query($conn, "SELECT * FROM actor JOIN actorm ON actor.a_id = actorm.a_id WHERE actorm.m_id ={$m_id}");
