...
<body>
    ...
    <section class="spad">
        <div class="container">
            ...
            <div class="row">
                <?php
                    $sqlM = "SELECT * FROM movie JOIN listm ON listm.m_id = movie.m_id WHERE listm.l_id='$l_id'";
                    $resultM = mysqli_query($conn, $sqlM);
                    if (mysqli_num_rows($resultM) > 0){
                    
                    while($row = mysqli_fetch_assoc($resultM)){
                        $sqlG = "SELECT * FROM genre JOIN genrem ON genrem.g_id = genre.g_id WHERE genrem.m_id =".$row['m_id']."";
