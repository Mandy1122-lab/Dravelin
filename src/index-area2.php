...
<body>
    ...
    <section id="content">
        ...
        <h5><span class="bold-text">編輯 - 小專區2</span></h5>
        <hr>
        <!-- Input Fields -->
        <script>
        ...
        $result = mysqli_query($link, "SELECT * FROM area WHERE area_id = 2"); // 根據“小專區2”的ID查詢記錄
        ...
        <form id="updateArea">
            ...
            <div style="margin-top: 20px;">
                ...
                <select name="area_choose">
                    <?php
                    // 合併查詢，並加上來源標籤
                    $query = "(SELECT sc_id, sc_name FROM scomplication)";
                    $listResult = mysqli_query($link, $query);
                    while ($list = mysqli_fetch_assoc($listResult)) {
                        // 假設每個選項的 value 僅需要 sc_id，如果需要其他格式請自行調整
                        $value = 'list.php?sc_id=' . $list['sc_id'];
                        // 檢查是否需要預選這個選項
                        $selected = ($value == $row['area_choose']) ? ' selected' : '';
                        // 顯示選項，選項文本使用 sc_name 字段
                        echo '<option value="' . htmlspecialchars($value) . '"' . $selected . '>' . htmlspecialchars($list['sc_name']) . '</option>';
                    }
                    ?>
                </select>
            ...
            <input type="hidden" name="area_id" value="2"> <!-- 這裡將其設置為2表示“小專區2” -->
